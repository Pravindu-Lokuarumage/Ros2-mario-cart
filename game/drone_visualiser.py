...
class RobotVisualiser(Node):
    ...
    def listener_callback(self, msg):
        ...
        msg1 = String()
        msg1.data = "0" 
        if ( msg.linear.x > 2.5 and msg.linear.x < 3.5 and msg.linear.y > -0.5 and msg.linear.y < 0.5):
             msg1.data = "1" 
        if (msg.linear.x > 5.5 and msg.linear.x < 6.5 and  msg.linear.y > 1.5 and msg.linear.y < 2.5):
             msg1.data = "2"     
        
        self.publisher.publish(msg1)
        self.get_logger().info("Publishing to /player/%s/pose" % self.player)
        #now move the object
